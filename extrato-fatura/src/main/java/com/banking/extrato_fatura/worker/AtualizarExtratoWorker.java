...
public class AtualizarExtratoWorker {
    ...
    public void subscribe() {
        ...
        log.info("Worker 'atualizar-extrato' iniciado");
    }
    ...
    private void handleTask(ExternalTask task, ExternalTaskService service) {
        ...
        try {
            log.info("Processando extrato - Task: {}", task.getId());
            ...
            extratoService.adicionarItem(contaId, transacaoId, comerciante, localizacao, valor, dataHora);

            log.info("Extrato atualizado - Conta: {}, Transação: {}", contaId, transacaoId);

            service.complete(task);

        } catch (Exception e) {
            log.error("Erro ao atualizar extrato - Conta: {}, Task: {}", contaId, task.getId(), e);
            service.handleFailure(task, "Erro ao atualizar extrato", e.getMessage(), maxRetries, retryDelay);
        }
    }
}